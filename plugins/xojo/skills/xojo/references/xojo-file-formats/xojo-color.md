...
Color assets are text files containing one `ColorGroup` symbol and one or more platform/appearance representations. The 17 controlled files in Additional Example 15 each contain one `Color` record; older corpus files demonstrate that a group can contain multiple records.
...
Controlled projects saved by the same IDE establish the current `Platform` values:
...
For `Platform=2`, the provider is carried by `Name`, not by a separate iOS or Android platform number. The controlled iOS and Android projects can each contain both Apple names and `Android|...` names, and both serialize with `Platform=2`.
