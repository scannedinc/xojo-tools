...
The older `Enum` layout repeats the enumeration name before `flag` as well as after `binE`; the two values are identical in all 42 observed groups across 11 example binaries. `binE` is not unconditional in either layout: the 2020.02 `Controlling Row Selection` enumeration omits it. A conversion retains whichever layout the input expresses instead of rewriting one into the other.
...
A text -> binary -> text conversion has the same normalization plus text-file formatting and manifest-ID normalization. For example, a text item ID written as `&hFFFFFFFFB934E1B5` can return as `&h00000000B934E1B5` because RbBF stores only the low 32-bit pattern. These spellings identify the same stored bit pattern, but the text bytes differ.
